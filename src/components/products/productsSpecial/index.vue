    ...
    <div class="flex flex-col w-full layout-container">
        <div v-if="preferentialProducts.length > 0" class="product-section flex flex-col max-w-full mb-5">
            ...
            <div class="product-container flex overflow-x-auto py-4 space-x-4 max-w-full" ref="preferentialProductContainer"
                @scroll="handleScroll(preferentialProductContainer, showLeftArrowPreferential, showRightArrowPreferential)">
            ...
            <button v-if="showRightArrowPreferential && vouchers.length > 6" @click="scrollRight(preferentialProductContainer)"
        ...
        <div v-if="suggestedProducts.length > 0" class="product-section flex flex-col max-w-full mb-5">
            ...
            <div class="product-container flex overflow-x-auto py-4 space-x-4 max-w-full" ref="suggestProductContainer"
                @scroll="handleScroll(suggestProductContainer, showLeftArrowSuggest, showRightArrowSuggest)">
                <!-- <div v-for="(voucher, index) in vouchers.slice(0, 6)" :key="voucher.id"
                    class="voucher-item bg-white p-4 rounded-lg shadow-lg flex-shrink-0 w-64">
                    <div class="voucher-header flex justify-between items-center">
                        <h3 class="font-bold text-black">Voucher {{ voucher.value / 1000 }}K</h3>
            ...
            <button v-if="showRightArrowSuggest && vouchers.length > 6" @click="scrollRight(suggestProductContainer)"
                class="btn-scroll absolute right-0 top-1/2 transform -translate-y-1/2 p-2 rounded-full">
                →
            </button>
        </div>
        ...
        <div class="flex-1">
            ...
            <div class="sort-filter">
                ...
                <select id="sort" @change="handleSortChange($event)" class="w-1/4">
...
onMounted(async () => {
    handleScroll(preferentialProductContainer.value, showLeftArrowPreferential.value, showRightArrowPreferential.value);
    handleScroll(suggestProductContainer.value, showLeftArrowSuggest.value, showRightArrowSuggest.value);
    fetchProducts();
...
const showLeftArrowPreferential = ref(false);
const showRightArrowPreferential = ref(true);
...
const handleScroll = (container: HTMLElement | null, showLeftArrow: boolean, showRightArrow: boolean) => {
    if (container) {
